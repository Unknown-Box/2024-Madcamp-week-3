...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return const MaterialApp(
      ...
      home: ExpenditureWordCloud(),
    );
  }
}

class ExpenditureWordCloud extends StatefulWidget {
  const ExpenditureWordCloud({super.key});

  @override
  _ExpenditureWordCloudState createState() => _ExpenditureWordCloudState();
}

class _ExpenditureWordCloudState extends State<ExpenditureWordCloud> {
  List<List<dynamic>> data = [];
  Map<int, Map<String, int>> weeklyTitleData = {};
  ...
  void processCSVData(List<List<dynamic>> csvData) {
    ...
    for (var i = 1; i < csvData.length; i++) {
      ...
      if (weekNumber == 52) weekNumber--;
      if (!weeklyTitleData.containsKey(weekNumber)) {
        weeklyTitleData[weekNumber] = {};
        weeks.add(weekNumber);
      }

      if (!weeklyTitleData[weekNumber]!.containsKey(title)) {
        weeklyTitleData[weekNumber]![title] = 0;
      }

      weeklyTitleData[weekNumber]![title] = weeklyTitleData[weekNumber]![title]! + value;
    }
    ...
    setState(() {
      data = csvData;
      print('Data processing complete. Total weeks: ${weeks.length}');
    });
  }
  ...
  String getColorForTitle(String title) {
    ...
    int hue = hash % 360;
    return 'hsl($hue, 59%, 50%)';
  }

  String getWordCloudOption(int week) {
    print('Generating word cloud for week: $week');
    var data = weeklyTitleData[week]!.entries.map((entry) {
      return {
        'name': entry.key,
        'value': entry.value,
        'textStyle': {
          'color': getColorForTitle(entry.key),
        }
      };
    }).toList();

    String dataToString = jsonEncode(data);
    print('Word cloud data for week $week: $dataToString');
        ...
        elements: [
          {
            ...
            top: '5%',
            style: {
              text: 'Week ${_currentWeekIndex.toString()}',
  ...
  Widget build(BuildContext context) {
    ...
    r.extendChartView({
      ...
      render:function(t,e,a){
        ...
        var o=t.getData(),n=t.get("gridSize");
        
        var randomDisplacement = function() {
          var displacementRange = 10;
          return {
            x: Math.random() * displacementRange - displacementRange / 2,
            y: Math.random() * displacementRange - displacementRange / 2
          };
        };
        
        var isWithinBounds = function(x, y, width, height) {
          return x >= 0 && y >= 0 && x + width <= window.innerWidth && y + height <= window.innerHeight;
        };
    
        var doesOverlap = function(el, otherEls) {
          var rect1 = el.getBoundingRect();
          for (var i = 0; i < otherEls.length; i++) {
            var rect2 = otherEls[i].getBoundingRect();
    ...
    t.layoutInstance.ondraw = function(e, a, s, l) {
      ...
      if(!doesOverlap(f, placedElements) && isWithinBounds(f.x, f.y, f.style.width, f.style.height)) {
      
        f.__floatingClass = "floating-word";
        
        f.setStyle('transform', '--random-x: ${displacement.x}px; --random-y: ${displacement.y}px;');
      
        i.add(f);
        placedElements.push(f);
        
        o.setItemGraphicEl(s, f);
        
        f.ensureState("emphasis").style = r.helper.createTextStyle(d.getModel(["emphasis", "textStyle"]), { state: "emphasis" });
        f.ensureState("blur").style = r.helper.createTextStyle(d.getModel(["blur", "textStyle"]), { state: "blur" });
  ...
  style.innerHTML = `
    .floating-word {
      animation: randomDisplacement 1s infinite alternate ease-in-out;
    }
    
    @keyframes randomDisplacement {
      0% {
        ...
        children: [
          Expanded(
            child: weeks.isEmpty
                ...
                : Echarts(
              extensions: const [wordCloudPlugin],
              option: getWordCloudOption(weeks[_currentWeekIndex]),
            ),
          ),
          if (weeks.isNotEmpty)
            Padding(
              ...
              child: Column(
                children: [
                  Slider(
                    value: _currentWeekIndex.toDouble(),
                    min: 0,
                    max: weeks.length - 1,
                    divisions: weeks.length - 1,
                    label: 'Week ${(weeks[_currentWeekIndex] + 1).toString()}',
                    onChanged: (value) {
                      setState(() {
                        _currentWeekIndex = value.toInt();
                        print('Week index changed to $_currentWeekIndex');
                      });
                    },
