...
class ExpenditureWordCloud extends StatefulWidget {
  ...
  @override
  State<ExpenditureWordCloud> createState() => _ExpenditureWordCloudState();
}
...
class _ExpenditureWordCloudState extends State<ExpenditureWordCloud> {
  ...
  Widget build(BuildContext context) {
    ...
    r.extendChartView({
      ...
      render:function(t,e,a){
        ...
        t.layoutInstance.ondraw=function(e,a,s,l){
          ...
          var u=d.getModel("textStyle");
          
          var f=new r.graphic.Text({
    ...
    function addFloatingAnimation(element) {
      ...
      let positionV = 0;
      let positionH = 0;
      const maxTranslationV = 0.6;
      const maxTranslationH = Math.random() / 8;
      const speedV = 0.01;
      const speedH = 0.002;
  
      function animate() {
          positionV += directionV * speedV;
          positionH += directionH * speedH;
          if (positionV >= maxTranslationV || positionV <= -maxTranslationV) {
              directionV *= -1; // Reverse direction
          }
          if (positionH >= maxTranslationH || positionH <= -maxTranslationH) {
              directionH *= -1; // Reverse direction
          }
          element.attr('position', [element.x + positionH, element.y + positionV]);
